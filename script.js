/*TODO доработать отмену линии целиком, а не по пикселям-ячейкам*/
...
// Массив хранения предыдущих состояний пиксельного поля
let HISTORY = []

// Функция для записи текущего состояния в историю
function saveCurrentState() {
    let currentState = '';
    let tempCells = document.querySelectorAll('.cell');
    for (let i = 0; i < tempCells.length; i++) {
        currentState += tempCells[i].dataset.color;
    }
    HISTORY.push(currentState); // Добавляем текущее состояние в историю
}

// Функционал отката назад
document.querySelector('.undo-tool').addEventListener('click', function() {
    if(HISTORY.length > 0) {
        let lastState = HISTORY.pop(); // Берём последнее состояние из истории
        let tempCells = document.querySelectorAll('.cell');
        for(let i = 0; i < tempCells.length; i++) {
            tempCells[i].style.backgroundColor = COLORS[parseInt(lastState[i])];
            tempCells[i].dataset.color = lastState[i]; // Восстанавливаем данные цвета каждой клетки
        }
    }
});
...
// Элемент интерфейса для отмены
let undoTool = document.createElement('div');
undoTool.className = 'tool-cell undo-tool';
undoTool.innerHTML = '<span class="material-icons">undo</span>';
document.querySelector('.tools').appendChild(undoTool); // Разместим новый инструмент рядом с остальными инструментами
